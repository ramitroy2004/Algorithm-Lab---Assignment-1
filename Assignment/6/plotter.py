...
if choice == 1:
    data = pd.read_csv(r'C:\Users\mindl\Desktop\AlgoLab\4\observation_uniform.csv')
elif choice == 2:
    data = pd.read_csv(r'C:\Users\mindl\Desktop\AlgoLab\4\observation_normal.csv')

elements = data['Input size'].to_numpy()
comp = data["Comparisons"].to_numpy()
time = data["Time (ms)"].to_numpy()

comp_ratio = comp / (elements * elements)
time_ratio = time / (elements * elements)
...
ax1.set_ylabel(r'$\frac{time}{n^2}$')
...
ax2.set_ylabel(r'$\frac{time}{n^2}$')
