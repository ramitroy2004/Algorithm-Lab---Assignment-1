...
int partition(int arr[], int start, int final, long *count)
{
...
void quickSort(int *arr, int start, int final, long *count)
{
    if (start < final)
    {
        int pos_of_pivot = partition(arr, start, final, count);
        quickSort(arr, start, pos_of_pivot, count);
...
int main()
{
    ...
    FILE* fp2 = fopen("observation_normal_n.csv", "w");
